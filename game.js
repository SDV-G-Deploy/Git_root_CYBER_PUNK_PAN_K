'use strict';

const CONFIG = Object.freeze({
  ARENA: {
    WIDTH: 960,
    HEIGHT: 540,
    BACKGROUND: '#0c1f2b',
    BORDER: '#26465a'
  },
  SHOOTER: {
    X: 100,
    Y: 270,
    RADIUS: 10,
    COLOR: '#9be7ff'
  },
  PROJECTILE: {
    SPEED: 620,
    RADIUS: 7,
    DRAG_PER_SECOND: 0.18,
    MIN_SPEED: 24,
    MAX_LIFETIME: 2.8,
    COLOR: '#ffd75e'
  },
  NODES: {
    RADIUS: 20,
    HIT_SCORE: 100,
    TYPES: {
      bomb: {
        color: '#ff5d73',
        label: 'B'
      },
      pusher: {
        color: '#5cc8ff',
        label: 'P'
      },
      multiplier: {
        color: '#9bff8a',
        label: 'M'
      }
    },
    RESOLVED_COLOR: '#3a4751'
  },
  LEVEL: {
    SHOTS: 1,
    LAYOUT: [
      { id: 'n1', type: 'bomb', x: 560, y: 160 },
      { id: 'n2', type: 'pusher', x: 710, y: 230 },
      { id: 'n3', type: 'multiplier', x: 640, y: 330 },
      { id: 'n4', type: 'bomb', x: 810, y: 320 }
    ]
  },
  TRAJECTORY: {
    STEPS: 36,
    STEP_TIME: 0.05,
    WIDTH: 2,
    DASH_PATTERN: [7, 7],
    COLOR: '#b5f3ff'
  },
  SIMULATION: {
    FIXED_DT: 1 / 120,
    MAX_DT: 0.25,
    MAX_STEPS: 12,
    MS_THRESHOLD: 10
  },
  EPSILON: 0.0001
});

let state = null;
let canvasRef = null;
let ctxRef = null;
let scoreLabelRef = null;
let shotsLabelRef = null;
let retryButtonRef = null;

function clamp(value, min, max) {
  return Math.min(max, Math.max(min, value));
}

function distance(ax, ay, bx, by) {
  return Math.hypot(ax - bx, ay - by);
}

function normalize(x, y) {
  const length = Math.hypot(x, y);
  if (length <= CONFIG.EPSILON) {
    return null;
  }

  return { x: x / length, y: y / length };
}

function createNodes() {
  return CONFIG.LEVEL.LAYOUT.map((node) => ({
    id: node.id,
    type: node.type,
    x: node.x,
    y: node.y,
    radius: CONFIG.NODES.RADIUS,
    resolved: false
  }));
}

function createState() {
  return {
    score: 0,
    shotsRemaining: CONFIG.LEVEL.SHOTS,
    phase: 'aim',
    projectile: null,
    nodes: createNodes(),
    aim: {
      x: CONFIG.SHOOTER.X + 120,
      y: CONFIG.SHOOTER.Y,
      active: false
    },
    accumulator: 0
  };
}

function getCanvasPoint(canvas, event) {
  const rect = canvas.getBoundingClientRect();
  const scaleX = canvas.width / Math.max(rect.width, 1);
  const scaleY = canvas.height / Math.max(rect.height, 1);

  return {
    x: clamp((event.clientX - rect.left) * scaleX, 0, CONFIG.ARENA.WIDTH),
    y: clamp((event.clientY - rect.top) * scaleY, 0, CONFIG.ARENA.HEIGHT)
  };
}

function updateHud() {
  if (!scoreLabelRef || !shotsLabelRef || !state) {
    return;
  }

  scoreLabelRef.textContent = `Score: ${state.score}`;
  shotsLabelRef.textContent = `Shots: ${state.shotsRemaining}`;
}

function findHitNode(projectile) {
  for (let i = 0; i < state.nodes.length; i += 1) {
    const node = state.nodes[i];
    if (node.resolved) {
      continue;
    }

    const hitRadius = node.radius + projectile.radius;
    if (distance(projectile.x, projectile.y, node.x, node.y) <= hitRadius) {
      return node;
    }
  }

  return null;
}

function fireShot(targetX, targetY) {
  if (!state || state.phase !== 'aim' || state.shotsRemaining <= 0) {
    return false;
  }

  const direction = normalize(targetX - CONFIG.SHOOTER.X, targetY - CONFIG.SHOOTER.Y);
  if (!direction) {
    return false;
  }

  state.projectile = {
    x: CONFIG.SHOOTER.X,
    y: CONFIG.SHOOTER.Y,
    vx: direction.x * CONFIG.PROJECTILE.SPEED,
    vy: direction.y * CONFIG.PROJECTILE.SPEED,
    radius: CONFIG.PROJECTILE.RADIUS,
    age: 0,
    alive: true
  };

  state.shotsRemaining -= 1;
  state.phase = 'simulate';
  updateHud();
  return true;
}

function resetLevel() {
  state = createState();
  updateHud();
}

function simulateProjectile(dt) {
  const projectile = state.projectile;
  if (!projectile || !projectile.alive) {
    state.phase = 'end';
    return;
  }

  projectile.age += dt;

  const drag = Math.max(0, 1 - CONFIG.PROJECTILE.DRAG_PER_SECOND * dt);
  projectile.vx *= drag;
  projectile.vy *= drag;

  projectile.x += projectile.vx * dt;
  projectile.y += projectile.vy * dt;

  const speed = Math.hypot(projectile.vx, projectile.vy);
  const outOfBounds =
    projectile.x < 0 ||
    projectile.x > CONFIG.ARENA.WIDTH ||
    projectile.y < 0 ||
    projectile.y > CONFIG.ARENA.HEIGHT;

  if (
    outOfBounds ||
    projectile.age >= CONFIG.PROJECTILE.MAX_LIFETIME ||
    speed <= CONFIG.PROJECTILE.MIN_SPEED
  ) {
    projectile.alive = false;
    state.phase = 'end';
    return;
  }

  const hitNode = findHitNode(projectile);
  if (hitNode) {
    hitNode.resolved = true;
    state.score += CONFIG.NODES.HIT_SCORE;
    projectile.alive = false;
    state.phase = 'end';
    updateHud();
  }
}

function predictTrajectory(targetX, targetY) {
  const direction = normalize(targetX - CONFIG.SHOOTER.X, targetY - CONFIG.SHOOTER.Y);
  if (!direction) {
    return [];
  }

  const points = [{ x: CONFIG.SHOOTER.X, y: CONFIG.SHOOTER.Y }];

  let x = CONFIG.SHOOTER.X;
  let y = CONFIG.SHOOTER.Y;
  let vx = direction.x * CONFIG.PROJECTILE.SPEED;
  let vy = direction.y * CONFIG.PROJECTILE.SPEED;

  for (let i = 0; i < CONFIG.TRAJECTORY.STEPS; i += 1) {
    const drag = Math.max(0, 1 - CONFIG.PROJECTILE.DRAG_PER_SECOND * CONFIG.TRAJECTORY.STEP_TIME);
    vx *= drag;
    vy *= drag;

    x += vx * CONFIG.TRAJECTORY.STEP_TIME;
    y += vy * CONFIG.TRAJECTORY.STEP_TIME;

    if (x < 0 || x > CONFIG.ARENA.WIDTH || y < 0 || y > CONFIG.ARENA.HEIGHT) {
      break;
    }

    points.push({ x, y });

    for (let n = 0; n < state.nodes.length; n += 1) {
      const node = state.nodes[n];
      if (node.resolved) {
        continue;
      }

      if (distance(x, y, node.x, node.y) <= CONFIG.PROJECTILE.RADIUS + node.radius) {
        return points;
      }
    }
  }

  return points;
}

function update(dt) {
  if (!state || state.phase !== 'simulate') {
    return;
  }

  let delta = Number(dt) || 0;
  if (delta > CONFIG.SIMULATION.MS_THRESHOLD) {
    delta /= 1000;
  }

  delta = clamp(delta, 0, CONFIG.SIMULATION.MAX_DT);
  state.accumulator += delta;

  let steps = 0;
  while (state.accumulator >= CONFIG.SIMULATION.FIXED_DT && steps < CONFIG.SIMULATION.MAX_STEPS) {
    simulateProjectile(CONFIG.SIMULATION.FIXED_DT);
    state.accumulator -= CONFIG.SIMULATION.FIXED_DT;
    steps += 1;
  }
}

function drawArena(ctx) {
  ctx.fillStyle = CONFIG.ARENA.BACKGROUND;
  ctx.fillRect(0, 0, CONFIG.ARENA.WIDTH, CONFIG.ARENA.HEIGHT);

  ctx.lineWidth = 2;
  ctx.strokeStyle = CONFIG.ARENA.BORDER;
  ctx.strokeRect(0, 0, CONFIG.ARENA.WIDTH, CONFIG.ARENA.HEIGHT);
}

function drawShooter(ctx) {
  ctx.beginPath();
  ctx.arc(CONFIG.SHOOTER.X, CONFIG.SHOOTER.Y, CONFIG.SHOOTER.RADIUS, 0, Math.PI * 2);
  ctx.fillStyle = CONFIG.SHOOTER.COLOR;
  ctx.fill();
}

function drawNodes(ctx) {
  ctx.font = 'bold 12px monospace';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';

  for (let i = 0; i < state.nodes.length; i += 1) {
    const node = state.nodes[i];
    const typeCfg = CONFIG.NODES.TYPES[node.type];

    ctx.beginPath();
    ctx.arc(node.x, node.y, node.radius, 0, Math.PI * 2);
    ctx.fillStyle = node.resolved ? CONFIG.NODES.RESOLVED_COLOR : typeCfg.color;
    ctx.fill();

    ctx.fillStyle = '#0c1f2b';
    ctx.fillText(typeCfg.label, node.x, node.y);
  }
}

function drawProjectile(ctx) {
  if (!state.projectile || !state.projectile.alive) {
    return;
  }

  ctx.beginPath();
  ctx.arc(state.projectile.x, state.projectile.y, state.projectile.radius, 0, Math.PI * 2);
  ctx.fillStyle = CONFIG.PROJECTILE.COLOR;
  ctx.fill();
}

function drawAimPreview(ctx) {
  if (state.phase !== 'aim' || !state.aim.active || state.shotsRemaining <= 0) {
    return;
  }

  const points = predictTrajectory(state.aim.x, state.aim.y);
  if (points.length < 2) {
    return;
  }

  ctx.save();
  ctx.strokeStyle = CONFIG.TRAJECTORY.COLOR;
  ctx.lineWidth = CONFIG.TRAJECTORY.WIDTH;
  ctx.setLineDash(CONFIG.TRAJECTORY.DASH_PATTERN);

  ctx.beginPath();
  ctx.moveTo(points[0].x, points[0].y);

  for (let i = 1; i < points.length; i += 1) {
    ctx.lineTo(points[i].x, points[i].y);
  }

  ctx.stroke();
  ctx.restore();
}

function drawEndHint(ctx) {
  if (state.phase !== 'end') {
    return;
  }

  const allResolved = state.nodes.every((node) => node.resolved);
  const message = allResolved ? 'Hit confirmed. Retry for next run.' : 'No more shots. Retry.';

  ctx.font = 'bold 20px monospace';
  ctx.fillStyle = allResolved ? '#8dffba' : '#ff9fa9';
  ctx.fillText(message, 280, 40);
}

function render(ctx) {
  if (!ctx || !state) {
    return;
  }

  ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);

  drawArena(ctx);
  drawAimPreview(ctx);
  drawShooter(ctx);
  drawNodes(ctx);
  drawProjectile(ctx);
  drawEndHint(ctx);
}

function frame(now) {
  if (!state) {
    return;
  }

  if (!frame.lastTime) {
    frame.lastTime = now;
  }

  const dt = (now - frame.lastTime) / 1000;
  frame.lastTime = now;

  update(dt);
  render(ctxRef);
  requestAnimationFrame(frame);
}

function initScene() {
  canvasRef = document.getElementById('chainlab-canvas');
  scoreLabelRef = document.getElementById('scoreLabel');
  shotsLabelRef = document.getElementById('shotsLabel');
  retryButtonRef = document.getElementById('retryButton');

  if (!canvasRef) {
    throw new Error('Canvas element #chainlab-canvas not found.');
  }

  ctxRef = canvasRef.getContext('2d');

  canvasRef.addEventListener('mousemove', (event) => {
    if (!state) {
      return;
    }

    const point = getCanvasPoint(canvasRef, event);
    state.aim.x = point.x;
    state.aim.y = point.y;
    state.aim.active = true;
  });

  canvasRef.addEventListener('mouseleave', () => {
    if (!state) {
      return;
    }

    state.aim.active = false;
  });

  canvasRef.addEventListener('click', (event) => {
    const point = getCanvasPoint(canvasRef, event);
    fireShot(point.x, point.y);
  });

  if (retryButtonRef) {
    retryButtonRef.addEventListener('click', () => {
      resetLevel();
    });
  }

  resetLevel();
  requestAnimationFrame(frame);
}

window.ChainLabP0 = {
  CONFIG,
  resetLevel,
  fireShot,
  update,
  render
};

window.addEventListener('DOMContentLoaded', initScene);
